(function (global, factory) {
  ...
  const RISK_TIERS = {
    ...
    challenge: { id: "challenge", label: "도전", tone: "challenge", workload: 1.12, deadline: -1, cash: 1.18, reputation: 1.15, dropChance: .50, eventPressure: .12 },
    high: { id: "high", label: "고위험", tone: "high", workload: 1.25, deadline: -1, cash: 1.35, reputation: 1.30, dropChance: .60, eventPressure: .25 }
  };
  const REGULAR_MULTIPLIERS = [1.48, 1.68, 1.90, 2.12];
  const REGULAR_DEADLINE_CUTS = [2, 2, 2, 3];
  const BOSS_MULTIPLIERS = [1.55, 1.65, 1.78, 1.90];
  const BOSS_DEADLINE_CUTS = [4, 5, 6, 7];

  function clamp(value, min, max) { return Math.min(max, Math.max(min, value)); }
  function chapterIndex(project) { return clamp(Math.round(Number(project?.chapter) || 1), 1, 4) - 1; }

  function scaleProject(project = {}, { clears = 0, tutorial = false } = {}) {
    ...
    return {
      ...project,
      max: Math.round(project.workload * multiplier * progression),
      deadline: Math.max(project.boss ? 12 : 5, project.deadline - deadlineCut),
  ...
  return { RISK_TIERS, scaleProject, applyRisk, recommendedCoverage, eventProfile };
});
